...
trait Ki2GameFactory[T <: Ki2Like] extends IOFactoryLike {
  ...
  final def parseKi2String(s: String)(implicit stateCache: StateCache): T = parseKi2String(s, isFreeMode = false)

  final def parseKi2String(s: String, isFreeMode: Boolean)(implicit stateCache: StateCache): T = parseKi2String(toLines(s, Ki2Factory.normalizeString), isFreeMode)

  final def parseKi2String(lines: Lines, isFreeMode: Boolean)(implicit stateCache: StateCache): T = parseKi2String(toNonEmptyLines(lines), isFreeMode)

  def parseKi2String(lines: NonEmptyLines, isFreeMode: Boolean)(implicit stateCache: StateCache): T
